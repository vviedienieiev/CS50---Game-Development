...
function Brick:init(x, y)
    ...
    -- nil for simple bricks, 0 for unlocked block, 1 for locked block
    self.locked = nil
...
function Brick:render()
    if self.inPlay and self.locked == nil then
    ...
    elseif self.inPlay and self.locked then 
        love.graphics.draw(gTextures['main'], 
            -- multiply color by 4 (-1) to get our color offset, then add tier to that
            -- to draw the correct tier and color brick onto the screen
            gFrames['lockedBricks'][self.locked], self.x, self.y)
    end
end
